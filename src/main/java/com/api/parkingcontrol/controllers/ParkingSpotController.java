...
public class ParkingSpotController {
    ...
    public ResponseEntity<Object> saveParkingSpot(@RequestBody @Valid ParkingSpotDto parkingSpotDto){
        if(parkingSpotService.existsByPlaca(parkingSpotDto.getPlaca())){
            return ResponseEntity.status(HttpStatus.CONFLICT).body("CONFLITO - Placa");
        }
        if(parkingSpotService.existsByNumeroVaga(parkingSpotDto.getNumeroVaga())){
            return ResponseEntity.status(HttpStatus.CONFLICT).body("CONFLITO - Vaga");
        }
        if(parkingSpotService.existsByApartamentoAndBloco(parkingSpotDto.getApartamento(), parkingSpotDto.getBloco())){
            return ResponseEntity.status(HttpStatus.CONFLICT).body("CONFLITO - Apartamento e Bloco");
        }
